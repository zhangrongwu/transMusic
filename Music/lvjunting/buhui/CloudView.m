...
-(void)touchUpInside:(UILabel *)ges
{
    NSLog(@"%@", ges.text);
...
-(NSArray *)containThree{
    ...
    CGSize width1=[(NSString *)ob1 boundingRectWithSize:CGSizeMake(1000, FONTHEIGHT) options:NSStringDrawingUsesFontLeading  attributes:attribute context:nil].size;
    ++tempIndex;

    
    
    NSObject *ob2=[dataARy objectAtModuloIndex:tempIndex];
    CGSize width2=[(NSString *)ob2 boundingRectWithSize:CGSizeMake(1000, FONTHEIGHT) options:NSStringDrawingUsesFontLeading  attributes:attribute context:nil].size;
    ++tempIndex;

    
    NSObject *ob3=[dataARy objectAtModuloIndex:tempIndex];
    CGSize width3=[(NSString *)ob3 boundingRectWithSize:CGSizeMake(1000, FONTHEIGHT) options:NSStringDrawingUsesFontLeading  attributes:attribute context:nil].size;
    
    NSLog(@"%f",width1.width);
...
//    CGFloat width3=[(NSString *)ob3 sizeWithFont:[UIFont systemFontOfSize:16] constrainedToSize:CGSizeMake(1000, FONTHEIGHT)].width;
    //如果三条显示的长度太长 就取两条显示
    if (width1.width+width2.width+width3.width>280) {
        //如果两条显示的长度太长 就取一条显示
        if (width1.width+width2.width>280) {
            ++index;
...
//        CGFloat thstrwidth=[thstr sizeWithFont:thfont constrainedToSize:CGSizeMake(1000, FONTHEIGHT)].width;
        ...
        CGSize thstrwidth=[thstr boundingRectWithSize:CGSizeMake(1000, FONTHEIGHT) options:NSStringDrawingUsesLineFragmentOrigin  attributes:attribute context:nil].size;
        
        
        
        BubbleV *label=[[BubbleV alloc] init];
        label.frame=CGRectMake(self.frame.size.width/2-thstrwidth.width/2, self.frame.size.height-FONTHEIGHT, thstrwidth.width,FONTHEIGHT);
        [label setText:thstr];//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
        label.delegate=self;
        [label sizeToFit];
        ...
        if ([temp count]==2) {
            if (i==0) {
                ...
                rect.origin.x=self.frame.size.width/4-thstrwidth.width/2;
                rect.origin.y-=FONTHEIGHT/2;
                label.frame=rect;
            }else if(i==1){
                //右边的
                CGRect rect=label.frame;
                rect.origin.x=self.frame.size.width/4*3-thstrwidth.width/2;
                rect.origin.y-=FONTHEIGHT/2;
                label.frame=rect;
            }
        }else{
            ...
            if (i==1) {
                ...
                rect.origin.x-=(thstrwidth.width-BUDDLEGAP_X);
        ...
        [path moveToPoint:CGPointMake(label.frame.origin.x+thstrwidth.width/2, label.frame.origin.y+FONTHEIGHT/2)];
        ...
        switch (i) {
            case 0:
            {
                if ([temp count]==1||[temp count]==3) {
                    //此时中间的走直线
                    [path addLineToPoint:CGPointMake(label.frame.origin.x+thstrwidth.width/2, FONTHEIGHT/2)];
                }else if([temp count]==2){
                    //没有中间 走左弧
                    [path addCurveToPoint:CGPointMake(label.frame.origin.x+thstrwidth.width/2, FONTHEIGHT/2) controlPoint1:CGPointMake(label.frame.origin.x+thstrwidth.width/2, label.frame.origin.y+FONTHEIGHT/2) controlPoint2:CGPointMake(0, self.frame.size.height/2)];
                }
            }
                break;
            case 1:
            {
                if ([temp count]==3) {
                    ...
                    //走右弧
                    [path addCurveToPoint:CGPointMake(label.frame.origin.x+thstrwidth.width/2, FONTHEIGHT/2) controlPoint1:CGPointMake(label.frame.origin.x+thstrwidth.width/2, label.frame.origin.y+FONTHEIGHT/2) controlPoint2:CGPointMake(self.frame.size.width, self.frame.size.height/2)];
...
-(void)reloadData:(NSArray *)ary{
    ...
    self.tim=[NSTimer scheduledTimerWithTimeInterval:FIREINTERVAL target:self selector:@selector(bubbleUp) userInfo:nil repeats:YES];
    [self.tim fire];
}
