...
//    _scrollView.contentInset = UIEdgeInsetsMake(_expandHeight, _expandWidth *2, 0, 0); //
    ...
    [_scrollView bringSubviewToFront:expandView];
...
//    CGFloat offsetX = scrollView.contentOffset.x;
    if(offsetY < _expandHeight * -1) {
        ...
        CGRect rect = _expandView.bounds;
        rect.size.width = -1*offsetY + 160;
        _expandView.bounds = rect;
...
- (void)reSizeView{
    ...
    //重置_expandView位置
    [_expandView setFrame:CGRectMake(0, -180, CGRectGetWidth(_expandView.frame) , _expandHeight)];
    
    NSLog(@"%@", NSStringFromCGRect(_expandView.frame));
//    CGRectGetWidth(_expandView.frame);
